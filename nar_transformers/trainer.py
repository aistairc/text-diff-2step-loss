...
class Trainer:
    ...
    def _inner_training_loop(
        ...
        tr_losses = {
            ...
            "ce": torch.tensor(0.0).to(args.device),
        }
        ...
        #beta_list = frange_cycle_zero_linear(max_steps, start=0.0, stop=1.0, n_cycle=1, ratio_increase=0.1, ratio_zero=0.1)
        #beta_list = np.full(max_steps, 1)

        beta_list = np.linspace(0, 1, max_steps)
        #beta_list *= 0.5
        #beta_list = [min(2*x, 1.) for x in beta_list]

        #beta_list = np.ones((max_steps,))
        #lin_steps = int(max_steps * 0.2)
        #beta_list[:lin_steps] = np.linspace(0, 1, lin_steps)
        ...
        for epoch in range(epochs_trained, num_train_epochs):
            ...
            for step, inputs in enumerate(epoch_iterator):
                ...
                with self.accelerator.accumulate(model):
                    ...
                    model.beta = beta_list[self.state.global_step]
    ...
    def _maybe_log_save_evaluate(self, tr_loss, tr_losses, beta, model, trial, epoch, ignore_keys_for_eval):
        if self.control.should_log:
            ...
            logs["loss_CE"] = round(tr_losses_scalar["ce"] / (self.state.global_step - self._globalstep_last_logged), 4)
            logs["beta"] = beta
    ...
    def training_step(self, model: nn.Module, inputs: Dict[str, Union[torch.Tensor, Any]], beta=0.0) -> torch.Tensor:
        ...
        with self.compute_loss_context_manager():
            ...
            #loss = outputs.loss + beta * outputs.loss_ce
            #loss = beta * outputs.loss + outputs.loss_ce
            loss = outputs.loss + outputs.loss_ce
            losses = {"ctc": outputs.loss.detach() / self.args.gradient_accumulation_steps,
                      "ce": outputs.loss_ce.detach() / self.args.gradient_accumulation_steps}
