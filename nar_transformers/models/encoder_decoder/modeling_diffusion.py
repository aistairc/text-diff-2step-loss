...
class DiffusionModel(PreTrainedModel):
    ...
    def __init__(
        ...
        #self.denoiser = copy.deepcopy(decoder)

        model_name = "google/bert_uncased_L-4_H-512_A-8" # small
        self.baseline = BaselineModel.from_encoder_decoder_pretrained(model_name, model_name)
        #self.baseline.load_state_dict(torch.load("/scratch/aae15163zd/outputs/nar/ctc-baseline.pt"))
        self.baseline.load_state_dict(torch.load("/scratch/aae15163zd/outputs/nar/ctc-baseline-token-crossatt.pt"))
    ...
    def forward(
        ...
        **kwargs,
        ...
        bs, msl = input_ids.shape
        device = input_ids.device
        visible_mask = attention_mask * 0 + 1

        encoder_outputs = self.baseline.encoder(
        ...
        encoder_cls = encoder_last_hidden_state[:, 0, :]
        encoder_cls_expd = encoder_cls.unsqueeze(1).repeat_interleave(msl, dim=1)

        logits = None
        # Diffusion
        if self.training:
            ts = torch.randint(0, self.num_diffusion_steps, (bs,), device=device)
            #noise = torch.randn_like(encoder_cls)
            #noisy_cls = self.noise_scheduler.add_noise(encoder_cls, noise, ts)
            #noisy_cls_expd = noisy_cls.unsqueeze(1).repeat_interleave(msl, dim=1)
            noise = torch.randn_like(encoder_last_hidden_state)
            noisy_all_hidden = self.noise_scheduler.add_noise(encoder_last_hidden_state, noise, ts)
            noisy_all_hidden = self.baseline.dropout(noisy_all_hidden)

            denoiser_outputs = self.decoder(
                timestep_ids=ts if self.use_time_emb else None,
                input_ids=input_ids,
                attention_mask=attention_mask,
                encoder_hidden_states=noisy_all_hidden, #None,
                encoder_attention_mask=visible_mask,
                #inputs_embeds=noisy_cls_expd,
                output_hidden_states=True,
                **kwargs_decoder,
            )
            #noise_pred = denoiser_outputs.hidden_states[-1][:, 0, :]
            noise_pred = denoiser_outputs.hidden_states[-1]
            loss_mse_fct = MSELoss()
            loss_ce = loss_mse_fct(noise_pred, noise)
            loss_ctc = torch.tensor(0.0, device=device)
        else:
            #latent = torch.randn_like(encoder_cls)
            latent = torch.randn_like(encoder_last_hidden_state)
            for ts in self.noise_scheduler.timesteps:
                #latent_expd = latent.unsqueeze(1).repeat_interleave(msl, dim=1)
                latent_drop = self.baseline.dropout(latent)
                denoiser_outputs = self.decoder(
                    timestep_ids=ts.to(device).unsqueeze(0) if self.use_time_emb else None,
                    input_ids=input_ids,
                    attention_mask=attention_mask,
                    encoder_hidden_states=latent_drop, #None,
                    encoder_attention_mask=visible_mask,
                    #inputs_embeds=latent_expd,
                    output_hidden_states=True,
                    **kwargs_decoder,
                )
                #noise_pred = denoiser_outputs.hidden_states[-1][:, 0, :]
                noise_pred = denoiser_outputs.hidden_states[-1]
                latent = self.noise_scheduler.step(noise_pred, ts, latent).prev_sample

            #latent_expd = latent.unsqueeze(1).repeat_interleave(msl, dim=1)
            latent = self.baseline.dropout(latent)
            decoder_outputs = self.baseline.decoder(
                #timestep_ids=ts.to(device).unsqueeze(0) if self.use_time_emb else None,
                input_ids=input_ids,
                attention_mask=attention_mask,
                encoder_hidden_states=latent, #None,
                encoder_attention_mask=visible_mask,
                #inputs_embeds=latent_expd,
                #inputs_embeds=latent_expd,
                output_hidden_states=True,
            ...
            #latent_pred = decoder_outputs.hidden_states[-1][:, 0, :]
