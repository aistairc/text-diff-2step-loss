...
class EncoderDecoderModel(PreTrainedModel):
    ...
    def __init__(
        ...
        self.d_decoder = decoder

        # for VAE
        self.latent_size = 16
        self.to_latent = nn.Linear(self.encoder.config.hidden_size, self.latent_size)
        self.from_latent = nn.Linear(self.latent_size, self.decoder.config.hidden_size)
        self.fc_mu = nn.Linear(self.encoder.config.hidden_size, self.latent_size)
        self.fc_var = nn.Linear(self.encoder.config.hidden_size, self.latent_size)
        #self.fc_mu = nn.Linear(self.encoder.config.hidden_size, self.encoder.config.hidden_size)
        #self.fc_var = nn.Linear(self.encoder.config.hidden_size, self.encoder.config.hidden_size)
    ...
    def forward(
        ...
        **kwargs,
        ...
        bs, msl, emb_size = encoder_hidden_states.size()
        max_pooled_state, _ = encoder_hidden_states.max(-1)
        cls_hidden_state = encoder_hidden_states[:, 0, :]
        cls_hidden_state_expd = cls_hidden_state.unsqueeze(1).repeat_interleave(msl, dim=1)
        ...
        # VAE
        if True:
            mu = self.fc_mu(encoder_hidden_states)
            log_var = self.fc_var(encoder_hidden_states)
            eps = torch.randn_like(log_var)
            if self.training:
                z = mu + torch.exp(log_var / 2) * eps
                loss_kl = - 0.5 * torch.mean(1 + log_var - mu.pow(2) - log_var.exp())
                #loss_kl = - 0.5 * (1 + log_var - mu.pow(2) - log_var.exp())
            ...
            z = self.fc_mu(encoder_hidden_states)
            loss_kl = torch.tensor(0.0)

        decoder_inputs = self.dropout(self.from_latent(z))
        #decoder_inputs = self.from_latent(z).unsqueeze(1).repeat_interleave(msl, dim=1)
        ...
        if labels is not None:
            ...
            input_lengths = visible_mask.sum(1)
            ...
            #loss = loss_fct(logits.reshape(-1, self.config.decoder.vocab_size), labels.view(-1))
