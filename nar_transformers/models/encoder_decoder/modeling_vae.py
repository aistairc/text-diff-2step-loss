...
class VAE(PreTrainedModel):
    ...
    def forward(
        ...
        **kwargs,
        ...
        embeds = self.encoder.bert.embeddings.word_embeddings(decoder_input_ids)
        #embeds = self.dropout(embeds)

        mu = self.fc_mu(embeds)
        log_var = self.fc_var(embeds)
        ...
        #embeds_pred = self.from_latent(F.relu(z))
        embeds_pred = self.from_latent(z)

        loss_fct = MSELoss()
        loss = loss_fct(embeds_pred, embeds)
        loss += loss_kl

        bs, msl, hidden_size = embeds.shape
        cdists = pairwise_euclidean_distance(embeds_pred.view(-1, hidden_size), self.embeds_table)
        logits = -1 * cdists.view(bs, msl, -1)
